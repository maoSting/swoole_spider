 ...
 * @package app\task
 */
class WuhanEr extends TaskAbstract {
    ...
    public function isContentUrl($url) {
        $match = preg_match('/^https:\/\/wh\.58\.com\/ershoufang\/[\da-z]{1,20}\.shtml/i', $url);
    ...
    public function isListUrl($url) {
        $match = preg_match('/^https:\/\/wh\.58\.com\/ershoufang\/pn\d{1,5}/i', $url);
    ...
    public function getUrl($html, $url) {
        ...
        if (!$nextDom instanceof SimpleHtmlDomNodeBlank) {
            ...
            if ($aHref) {
                $nextUrl[] = sprintf("https://wh.58.com%s", $aHref);
            }
        }
    ...
    public function saveContent(MysqlPool $mysqli, $data, $url) {
        if($data){
            ...
            $exist     = $mysqlTool->name('dq_wuhan_er')->where(['phone'   => $data['phone']])->find();
            if (empty($exist)) {
                Db::init($mysqli)->name('dq_wuhan_er')->insert($data);
            } else {
